...
function timesChar(string, character) {
	if (typeof string !== 'string' || typeof character !== 'string' || character.length !== 1) {
		console.error("Invalid input: both parameters must be strings and the second string must be a single character.");
		return -1;
	} else {
		let count = 0;
...
console.log(timesChar("Characteristic", "c")); // Output: 3
...
function isPalindrome(string) {
	if (typeof string !== 'string' || string.length < 1) {
		console.error("Invalid input: parameter must be a non-empty string.");
		return false;
	}
...
console.log(isPalindrome("abeceba")); // Output: true
...
const stringsArray = ["apple", "banana", "pear", "grape", "kiwi", "watermelon"];
const resultArray = stringsArray.filter(string => string.length >= 5).map(string => string.toUpperCase()).join("; ");
...
console.log(stringCompression("GGGHHRRRRRRRUIIIOOOO")); // Output: 3G2H7R1U3I4O
console.log(stringCompression("B"));
console.log(stringCompression(5));
...
const array = ["Statue", "Tree", "House", "Car", "Boat", "Plane"];
array.unshift("Mountain", "River");
array.push("Bike", "Bus");
array.splice(3, 3); // splice(index, numberOfElementsToRemove)
array.splice(array.length - 1, 0, "Bridge", "Tunnel"); // splice(index, 0, elementsToAdd)
